...
# gets participants by finding the correct semester, event, and then participants
def participants_for_event
  semester = Semester.where('season = ? AND year = ?', Semester.seasons[@bot_season], @bot_year).first
  event = semester.events.where('event_type = ?', Event.event_types[@bot_event]).first
  event.participants
end

# loops through all participants for event and returns ids
def all_participant_ids_for_event
  participant_ids = []
  participants_for_event.each do |participant|
