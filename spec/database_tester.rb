...
# to restore the single true state of the database
...
class DatabaseTester
  ...
  def process_person(person, role, event, r, person_hash, role_field_hash, 
  ...
  def validate_person_hash(person_hash, email, attribute_hash)
    ...
    attribute_hash.each do |attribute_key, arrays|
      if attribute_key.include?('form_id') || attribute_key.include?('submit_date')
        truth_array << validate_submission_form_arrays(person_hash, person, attribute_key)
      else
  ...
  def validate_submission_form_arrays(person_hash, person, attribute_key)
    ...
    database_array = person.send(attribute_key)
    if attribute_key == 'submit_date'
      typeform_array = map_datetime_parse_array(typeform_array)
      database_array = map_datetime_parse_array(database_array)
    end
    validate_arrays(typeform_array, database_array, person)
  end
  ...
  def clean_attribute_array(array)
    array.flatten.select {|value| value != nil}.map {|value| value.to_s.strip.downcase}
  end
